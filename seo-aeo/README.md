...
4. Publish Wave 1–11 from `publish/` into Ghost (`tools/publish_to_ghost.py` once `GHOST_ADMIN_API_KEY` is set). Paste Wave 5 onto `/largest-sovereign-wealth-funds/` **before** 301ing the year-stamped twins. Then Wave 6 (asset-owners title is still “(2025)”). Then Wave 7 (GPIF return is wrong on the live page). Then Wave 8 (CIC explained still says $1.3T). Then Wave 9 (NZ Super still NZ$71.4bn; ADIA still prints ~$1.1T). Then Wave 10 (NY Common still $226.8bn; Texas PSF still $8.3bn). Then Wave 11 (Chile ESSF still $10.8bn; Mumtalakat still a tracker AUM).
