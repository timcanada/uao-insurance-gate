...
4. Publish Wave 1–8 from `publish/` into Ghost (`tools/publish_to_ghost.py` once `GHOST_ADMIN_API_KEY` is set). Paste Wave 5 onto `/largest-sovereign-wealth-funds/` **before** 301ing the year-stamped twins. Then Wave 6 (asset-owners title is still “(2025)”). Then Wave 7 (GPIF return is wrong on the live page). Then Wave 8 (CIC explained still says $1.3T; CalSTRS funded status still 2023; Temasek-is-SWF still S$413bn).
