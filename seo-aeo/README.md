...
4. Publish Wave 1–7 from `publish/` into Ghost (`tools/publish_to_ghost.py` once `GHOST_ADMIN_API_KEY` is set). Paste Wave 5 onto `/largest-sovereign-wealth-funds/` **before** 301ing the year-stamped twins. Then Wave 6 (asset-owners title is still “(2025)”). Then Wave 7 (GPIF return is wrong on the live page).
